...
def DRCNet(D, N, filters_list, kernel_size_list, padding="same", img=Input((224,224,3))):
    encoded_img = EncoderNet()(img)
    drc_output = DRCN(D, N, filters_list, kernel_size_list, padding)(encoded_img)
...
# def GetDRC():
#     feat_abstract_model = DRCNet(D=3, N=5, filters_list=[32, 32, 32], kernel_size_list=[(3, 3), (3, 3), (3, 3)])
#     return Model(inputs=[feat_abstract_model.layers[0].input], outputs=[feat_abstract_model.layers[-1].output])
...
def IntentionNet(mode, input_frame, num_control, num_intentions=-1, use_side_model=True):
    ...
    if input_frame != "MULTI":
        ...
        feat_abstract_model = DRCNet(D=3, N=5, filters_list=[32, 32, 64], kernel_size_list=[(3, 3), (3, 3), (3, 3)], img=rgb_input)
        ...
        if use_side_model:
            feat_abstract_model_l = DRCNet(D=3, N=5, filters_list=[32, 32, 64], kernel_size_list=[(3, 3), (3, 3), (3, 3)], img=rgbl_input)
            feat_abstract_model = DRCNet(D=3, N=5, filters_list=[32, 32, 64], kernel_size_list=[(3, 3), (3, 3), (3, 3)], img=rgbm_input)
            feat_abstract_model_r = DRCNet(D=3, N=5, filters_list=[32, 32, 64], kernel_size_list=[(3, 3), (3, 3), (3, 3)], img=rgbr_input)
    ...
    if mode == "DLM":
        ...
        if mode == "LPE_SIAMESE":
            ...
            lpe_feat_abstract_model = DRCNet(D=3, N=5, filters_list=[32, 32, 64], kernel_size_list=[(3, 3), (3, 3), (3, 3)], img=lpe_intention_input)
...
if __name__ == "__main__":
    ...
    img = load_img("/home/zhuoran/IntentionNet_zhuoran/intention_net/test/dog.jpeg", target_size=(224, 224))
    img = preprocess_input(img_to_array(img))
    img = np.expand_dims(img, axis=0) # expand dims to make it a batch

    intention_net = IntentionNet("DLM", input_frame="NORMAL", num_control=2, num_intentions=4)
