...
"""
Every Sequence must implements the __getitem__ and the __len__ methods. If you want to modify your
dataset between epochs you may implement on_epoch_end. The method __getitem__ should return a complete batch.
"""
class BaseDataset(keras.utils.Sequence):
    NUM_CONTROL = 2
    def __init__(self, data_dir, batch_size, num_intentions, mode, target_size=(224, 224),
                 shuffle=False, max_samples=None, preprocess=True, input_frame="NORMAL"):
    ...
    def init(self):
        # you must assign the num_samples
...
class PioneerDataset(BaseDataset):
    ...
    def __init__(self, data_dir, batch_size, num_intentions, mode, target_size=(224, 224),
                 shuffle=False, max_samples=None, preprocess=True, input_frame="NORMAL"):
        super().__init__(data_dir, batch_size, num_intentions, mode, target_size,
                 shuffle, max_samples, preprocess, input_frame)

    def init(self):
        base_dir = os.path.join(self.data_dir, "data")
        path_to_file = os.path.join(base_dir, "label.txt")
        self.data_header, self.data = self.read_csv(path_to_file, has_header=True)
        # Initialize the number of training examples
        self.num_samples = len(self.data)

        # reverse header index
        self.data_idx = {}
        for idx, data_name in enumerate(self.data_header):
            self.data_idx[data_name] = idx

        # Store all paths to images corresponding to training samples
        self.images = []
        for data in self.data:
            # mid + left + right cam
            if self.input_frame == "MULTI":
                img_left_cam = os.path.join(base_dir, "rgb_6", f"{int(data[self.data_idx['frame']])}.jpg")
                img_mid_cam = os.path.join(base_dir, "rgb_0", f"{int(data[self.data_idx['frame']])}.jpg")
                img_right_cam = os.path.join(base_dir, "rgb_4", f"{int(data[self.data_idx['frame']])}.jpg")
                self.images.append([img_left_cam, img_mid_cam, img_right_cam])
            else:
                # only has front cam
                img_mid_cam = os.path.join(base_dir, "rgb_0", f"{int(data[self.data_idx['frame']])}.jpg")
                self.images.append(img_mid_cam)

    def __getitem__(self, index):
        """
        Generate one batch of data. Arg: index: here we are generating the No.index th batch
        """
        ...
        for idx in indices:
            ...
            if self.input_frame == "MULTI":
                # left + mid + right cam
                img_left = img_to_array(load_img(self.images[idx][0], target_size=self.target_size))
                img_mid = img_to_array(load_img(self.images[idx][1], target_size=self.target_size))
                img_right = img_to_array(load_img(self.images[idx][2], target_size=self.target_size))
                if self.preprocess:
                    img_left = preprocess_input(img_left)
                    img_mid = preprocess_input(img_mid)
                    img_right = preprocess_input(img_right)
                IMGL.append(img_left)
                IMGM.append(img_mid)
                IMGR.append(img_right)
            else:
                # only use front/mid cam
            ...
            if self.mode == "DLM":
                # data row (about dlm column) stores "forward"/"stop"/"left"/"right"
                data_intention_name = data[self.data_idx["dlm"]]
                if not data_intention_name:
                    data_intention_name = 'forward'
                data_intention_value = self.INTENTION_MAPPING[data_intention_name]
                data_intention = to_categorical(data_intention_value, num_classes=self.num_intentions)
                INTENT.append(data_intention)
            """
            else:
                # only add dlm support above, you can extend to lpe easily refering to the HuaweiFinalDataset example
            """
            # Get this data's control
            velocity = float(data[self.data_idx["current_velocity"]])
            angle = float(data[self.data_idx["steering_wheel_angle"]])
            control = [velocity/self.SCALE_VEL, angle/self.SCALE_STEER]
            CON.append(control)

        # Finished Generating the whole batch
        if self.mode == "MULTI":
            IMGL = np.array(IMGL)
            IMGM = np.array(IMGM)
            IMGR = np.array(IMGR)
            ...
            CON = np.array(CON)
            return [IMGL, IMGM, IMGR, INTENT], CON
        else:
            IMG = np.array(IMG)
            INTENT = np.array(INTENT)
            CON = np.array(CON)
            return [IMG, INTENT], CON
    ...
    def read_csv(self, fn, has_header=True):
        ...
        header = None
        data = []
        # Whether the file has a header row at the top
        if has_header:
            row_is_header = True
            for row in reader:
                if row_is_header:
